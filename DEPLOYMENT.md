...
This guide details how to deploy the hardened Sibna Server in a production environment.
...
### Build the Image
```bash
docker build -t sibna-server:latest .
```
...
```ini
[Unit]
Description=Sibna Protocol Server
...
Ensure all clients (`sibna-messenger.py`, etc.) point to your new IP:
```python
